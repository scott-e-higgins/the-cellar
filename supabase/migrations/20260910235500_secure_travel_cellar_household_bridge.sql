-- Travel Journal and Cellar keep separate household records. Existing explicit
-- Travel references establish the household bridge; both schemas' RLS still apply.
...
as $$
  ...
  bridged_cellar_households as (
    select distinct r.household_id
    from target_trip target
    join public.trips linked_trip
      on linked_trip.household_id = target.household_id
    join cellar.travel_references r
      on r.external_system = 'travel-journal'
     and r.external_entity_type = 'trip'
     and r.external_id = linked_trip.id::text
  ),
  explicit_visits as (
    select distinct
      v.id,
    ...
    join bridged_cellar_households bridge
      on bridge.household_id = r.household_id
  ...
  unlinked_visits as (
    ...
    join bridged_cellar_households bridge
      on bridge.household_id = v.household_id
    where not exists (
      select 1
  ...
  date_match_counts as (
    select v.id as visit_id, count(trip.id) as trip_count
    from unlinked_visits v
    join target_trip target on true
    join public.trips trip
      on trip.household_id = target.household_id
     and v.visit_date between trip.start_date and trip.end_date
    group by v.id
  ),
  inferred_visits as (
    select
    ...
    join unlinked_visits v
      on v.visit_date between t.start_date and t.end_date
