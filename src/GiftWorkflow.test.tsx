...
it('lost gift acknowledgement disables repeat gifts and refreshes without another decrement',async()=>{
 api.rpc.mockResolvedValueOnce({error:{message:'Network request failed'}});mount();fireEvent.change(screen.getByLabelText('Gifted to'),{target:{value:'Friend'}});fireEvent.submit(document.querySelector('form')!);await screen.findByRole('button',{name:'Refresh & Check'});fireEvent.submit(document.querySelector('form')!);expect(api.rpc).toHaveBeenCalledOnce();fireEvent.click(screen.getByRole('button',{name:'Refresh & Check'}));await waitFor(()=>expect(close).toHaveBeenCalledOnce());expect(api.rpc).toHaveBeenCalledOnce();expect(notice).toHaveBeenCalledWith(expect.stringContaining('could not be confirmed'),'warning')
})
