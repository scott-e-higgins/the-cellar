...
beforeEach(()=>{vi.clearAllMocks();api.rpc.mockImplementation(async(name)=>({data:name==='save_wine_entry'?{purchase_id:'purchase',wine_ids:['wine'],winery_id:'winery',visit_id:'visit',trip_id:'trip'}:'saved-id',error:null}));saved.mockResolvedValue(undefined)})
afterEach(cleanup)
...
it('new Wine continues to quantity/location and saves once with acquisition context',async()=>{
 acquire();change('Wine name','New Riesling');change('Vintage','2024');expect(api.rpc).not.toHaveBeenCalled();change('Quantity','3');change('Price per bottle (optional)','20');await submit();
 expect(api.rpc).toHaveBeenCalledWith('save_wine_entry',expect.objectContaining({p_definition_only:false,p_details:expect.objectContaining({acquisition_date:'2026-08-01',subtotal:60,total_cost:60,purchase_location:'Forge'}),p_lines:[expect.objectContaining({new_wine:expect.objectContaining({name:'New Riesling',vintage:2024,winery_id:'winery'}),quantity:3,storage_location_id:'rack'})]}));expect(close).toHaveBeenCalledOnce()
})
...
it('mixed visit purchase includes an existing wine and in-context new wine with shared trip/date',async()=>{
 acquire('record-purchase');change('Wine name','Riesling');fireEvent.click(screen.getByRole('button',{name:/Use Riesling/}));fireEvent.click(screen.getByRole('button',{name:'Add another wine before saving'}));fireEvent.change(screen.getAllByLabelText('Wine name')[1],{target:{value:'Missing Red'}});await submit();const payload=api.rpc.mock.calls[0][1];expect(payload.p_details).toMatchObject({visit_id:'visit',trip_id:'trip',acquisition_date:'2026-08-01'});expect(payload.p_lines).toHaveLength(2);expect(payload.p_lines[0].wine_id).toBe('wine');expect(payload.p_lines[1].new_wine).toMatchObject({name:'Missing Red',winery_id:'winery'});expect(api.rpc).toHaveBeenCalledOnce()
})
it('uncertain acquisition retry reuses the exact payload and id; successful refresh failure prevents another save',async()=>{
 api.rpc.mockResolvedValueOnce({error:{message:'Network failed'}});acquire('record-purchase');change('Wine name','Riesling');fireEvent.click(screen.getByRole('button',{name:/Use Riesling/}));await submit();expect(screen.getByRole('button',{name:'Retry Save'})).toBeTruthy();expect((screen.getByLabelText('Wine name') as HTMLInputElement).matches(':disabled')).toBe(true);saved.mockRejectedValueOnce(new Error('refresh failed'));await submit();expect(api.rpc.mock.calls[1][1]).toEqual(api.rpc.mock.calls[0][1]);expect(notice).toHaveBeenCalledWith('Saved, but the screen could not refresh.','warning');await submit();expect(api.rpc).toHaveBeenCalledTimes(2)
})
