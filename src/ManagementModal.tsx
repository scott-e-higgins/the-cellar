...
import { photosForVisit, tripForVisit, visitCanBeDeleted, winesForVisit } from './lib/visits'
...
export function ManagementModal({ target, householdId, data, photoUrls, editable, canGoBack, navigationDepth, returnToEnrichment, onBack, onClose, onNavigate, onVisitDeleted, onSaved, onNotice, onOpenBottle }: {
  target: ManagementTarget; householdId: string; data: CellarData; photoUrls: Record<string, string>; editable: boolean; canGoBack: boolean; navigationDepth: number; returnToEnrichment: boolean; onBack: () => void; onClose: () => void; onNavigate: (target: ManagementTarget) => void; onVisitDeleted: (wineryId: string) => void; onSaved: () => Promise<void>; onNotice: (message: string, tone?: NoticeTone) => void; onOpenBottle: (wineId: string) => void
}) {
  ...
  const viewPhoto = (photo: PhotoRecord) => {
    setViewer(photo)
    window.history.pushState({ ...window.history.state, cellarLightbox: true }, '')
  }
  ...
  return <>
    <ModalLayer layer="management" onDismiss={onClose} dismissible={!busy} surfaceClassName="workflow-modal workflow-form-modal management-modal" surfaceRef={modalRef} ariaLabel={title} onSurfaceScroll={(event) => { contextByTarget.current.set(currentKey, { scrollTop: event.currentTarget.scrollTop, tab }); if (target.kind === 'enrichment') { enrichmentScroll.current = event.currentTarget.scrollTop; sessionStorage.setItem('cellar.enrichment.scroll', String(event.currentTarget.scrollTop)) } }}>
      ...
      {recordTarget && tab === 'details' && <>
        ...
        {target.kind === 'visit' && <VisitDetails visit={target.record} data={data} editing={editing} setEditing={setEditing} editable={editable} busy={busy} setBusy={setBusy} setMessage={setMessage} householdId={householdId} onSaved={onSaved} onNotice={onNotice} onNavigate={navigateFromCurrent} onDeleted={onVisitDeleted} />}
      </>}
...
function RecordHero({ target, data, hasPhoto, url, editable, onView, onAdd }: { target: ManagementTarget; data: CellarData; hasPhoto: boolean; url?: string; editable: boolean; onView: () => void; onAdd: () => void }) {
  ...
  return <section className="record-hero"><button className={`record-hero-media ${hasPhoto ? 'has-photo' : editable ? 'can-add-photo' : ''}`} onClick={hasPhoto ? onView : editable ? onAdd : undefined} disabled={!hasPhoto && !editable} aria-label={hasPhoto ? 'View photo' : editable ? 'Add photo' : undefined}>{url ? <img src={url} alt="" /> : hasPhoto ? <small>Loading photo…</small> : <span>{fallback}</span>}</button><p>{meta}</p><strong>{summary}</strong></section>
}
...
function WineDetails({ wine, data, editing, setEditing, editable, busy, setBusy, setMessage, householdId, onSaved, onNotice, onNavigate, onOpenBottle, onEnrichmentAccepted }: { wine: WineRecord; data: CellarData; editing: boolean; setEditing: (value: boolean) => void; editable: boolean; busy: boolean; setBusy: (value: boolean) => void; setMessage: (value: string) => void; householdId: string; onSaved: () => Promise<void>; onNotice: (message: string, tone?: NoticeTone) => void; onNavigate: (target: ManagementTarget) => void; onOpenBottle: (wineId: string) => void; onEnrichmentAccepted?: () => void }) {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await finishSuccessfulAction({ refresh: onSaved, finish: () => setEditing(false), notice: onNotice, message: 'Wine saved.' })
    } catch (error) { setMessage(userError(error, 'The wine could not be saved. Please try again.')) } finally { setBusy(false) }
  }
  if (editing) return <form className="workflow-form detail-form edit-record-form" onSubmit={save}>
    ...
    <div className="form-actions edit-form-actions"><button type="button" className="secondary-button" onClick={() => setEditing(false)} disabled={busy}>Cancel</button><button className="primary-button" disabled={busy}>{busy ? 'Saving…' : 'Save wine'}</button></div>
  </form>
...
function VisitDetails({ visit, data, editing, setEditing, editable, busy, setBusy, setMessage, householdId, onSaved, onNotice, onNavigate, onDeleted }: { visit: VisitRecord; data: CellarData; editing: boolean; setEditing: (value: boolean) => void; editable: boolean; busy: boolean; setBusy: (value: boolean) => void; setMessage: (value: string) => void; householdId: string; onSaved: () => Promise<void>; onNotice: (message: string, tone?: NoticeTone) => void; onNavigate: (target: ManagementTarget) => void; onDeleted: (wineryId: string) => void }) {
  ...
  const relatedWines = winesForVisit(data, visit.id)
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await finishSuccessfulAction({ refresh: onSaved, finish: () => setEditing(false), notice: onNotice, message: 'Visit saved.' })
  ...
  if (editing) return <form className="workflow-form detail-form edit-record-form" onSubmit={save}>
    ...
    <div className="form-actions edit-form-actions"><button type="button" className="secondary-button" onClick={() => setEditing(false)} disabled={busy}>Cancel</button><button className="primary-button" disabled={busy}>{busy ? 'Saving…' : 'Save visit'}</button></div>
...
function PhotoPanel({ target, householdId, photos, urls, editable, busy, autoAdd, onPrompted, setBusy, setMessage, onSaved, onNotice, onView }: { target: ManagementTarget; householdId: string; photos: PhotoRecord[]; urls: Record<string, string>; editable: boolean; busy: boolean; autoAdd: boolean; onPrompted: () => void; setBusy: (value: boolean) => void; setMessage: (value: string) => void; onSaved: () => Promise<void>; onNotice: (message: string, tone?: NoticeTone) => void; onView: (photo: PhotoRecord) => void }) {
  ...
  const upload = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!supabase || !('record' in target)) return; const formElement = event.currentTarget, form = new FormData(formElement), file = form.get('photo'); if (!(file instanceof File) || !file.size) return; setBusy(true); setMessage(''); const parent = target.kind === 'wine' ? ['wines', 'wine_id'] : target.kind === 'winery' ? ['wineries', 'winery_id'] : target.kind === 'opening' ? ['openings', 'opening_id'] : target.kind === 'purchase' ? ['purchases', 'purchase_id'] : ['visits', 'winery_visit_id']; const path = `${householdId}/${parent[0]}/${target.record.id}/${createUniqueId()}-${safeName(file.name)}`; try { validatePhoto(file); const stored = await supabase.storage.from('cellar-photos').upload(path, file, { contentType: file.type, upsert: false }); if (stored.error) throw stored.error; const canBeHero = target.kind === 'wine' || target.kind === 'winery' || target.kind === 'visit'; const photographedOn = optional(form, 'photographed_at'); const result = await supabase.from('photos').insert({ household_id: householdId, [parent[1]]: target.record.id, storage_path: path, original_filename: file.name, mime_type: file.type, file_size_bytes: file.size, caption: optional(form, 'caption'), photographed_at: photographedOn ? `${photographedOn}T12:00:00` : null, is_hero: canBeHero && photos.length === 0 }); if (result.error) { await supabase.storage.from('cellar-photos').remove([path]); throw result.error } await finishSuccessfulAction({ refresh: onSaved, finish: () => { formElement.reset(); setAdding(false) }, notice: onNotice, message: 'Photo added.' }) } catch (error) { setMessage(userError(error, 'The photo could not be uploaded. Please try again.')) } finally { setBusy(false) } }
...
function Documents({ householdId, data, editable, onSaved, onNotice, onNavigate }: { householdId: string; data: CellarData; editable: boolean; onSaved: () => Promise<void>; onNotice: (message: string, tone?: NoticeTone) => void; onNavigate: (target: ManagementTarget) => void }) {
  const [busy, setBusy] = useState(false), [message, setMessage] = useState('')
  const upload = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const formElement = event.currentTarget; if (!supabase) return; setBusy(true); const form = new FormData(formElement), file = form.get('file'); try { if (!(file instanceof File) || !file.size) throw new Error('Choose a receipt or document.'); const purchaseId = String(form.get('purchase_id')), path = `${householdId}/purchases/${purchaseId}/${createUniqueId()}-${safeName(file.name)}`; const stored = await supabase.storage.from('cellar-documents').upload(path, file, { contentType: file.type, upsert: false }); if (stored.error) throw stored.error; const result = await supabase.from('documents').insert({ household_id: householdId, purchase_id: purchaseId, document_type: optional(form, 'document_type') || 'receipt', display_title: String(form.get('display_title')).trim(), document_date: optional(form, 'document_date'), storage_path: path, original_filename: file.name, mime_type: file.type, file_size_bytes: file.size }); if (result.error) { await supabase.storage.from('cellar-documents').remove([path]); throw result.error } await finishSuccessfulAction({ refresh: onSaved, finish: () => { formElement.reset(); formElement.closest('details')?.removeAttribute('open') }, notice: onNotice, message: 'Document uploaded.' }) } catch (error) { setMessage(userError(error, 'The document could not be uploaded. Please try again.')) } finally { setBusy(false) } }
