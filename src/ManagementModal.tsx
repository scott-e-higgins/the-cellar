...
export function ManagementModal({ target, householdId, data, photoUrls, editable, canGoBack, navigationDepth, returnToEnrichment, onBack, onClose, onNavigate, onSaved, onNotice, onOpenBottle }: {
  target: ManagementTarget; householdId: string; data: CellarData; photoUrls: Record<string, string>; editable: boolean; canGoBack: boolean; navigationDepth: number; returnToEnrichment: boolean; onBack: () => void; onClose: () => void; onNavigate: (target: ManagementTarget) => void; onSaved: () => Promise<void>; onNotice: (message: string, tone?: NoticeTone) => void; onOpenBottle: (wineId: string) => void
}) {
  ...
  return <>
    ...
    {viewer && <PhotoViewer photo={viewer} url={photoUrls[viewer.id]} onClose={closeViewer} />}
  </>
}
...
function WineryDetails({ winery, data, editing, setEditing, editable, busy, setBusy, setMessage, householdId, onSaved, onNotice, onNavigate, onEnrichmentAccepted }: { winery: WineryRecord; data: CellarData; editing: boolean; setEditing: (value: boolean) => void; editable: boolean; busy: boolean; setBusy: (value: boolean) => void; setMessage: (value: string) => void; householdId: string; onSaved: () => Promise<void>; onNotice: (message: string, tone?: NoticeTone) => void; onNavigate: (target: ManagementTarget) => void; onEnrichmentAccepted?: () => void }) {
...
function PhotoPanel({ target, householdId, photos, urls, editable, busy, autoAdd, onPrompted, setBusy, setMessage, onSaved, onNotice, onView }: { target: ManagementTarget; householdId: string; photos: PhotoRecord[]; urls: Record<string, string>; editable: boolean; busy: boolean; autoAdd: boolean; onPrompted: () => void; setBusy: (value: boolean) => void; setMessage: (value: string) => void; onSaved: () => Promise<void>; onNotice: (message: string, tone?: NoticeTone) => void; onView: (photo: PhotoRecord) => void }) {
  ...
  useEffect(() => { if (autoAdd && editable) requestAnimationFrame(() => { inputRef.current?.click(); onPrompted() }) }, [autoAdd, editable, onPrompted])
  const upload = async (file: File) => { if (!supabase || !('record' in target)) return; setBusy(true); setMessage(''); const parent = target.kind === 'wine' ? ['wines', 'wine_id'] : target.kind === 'winery' ? ['wineries', 'winery_id'] : target.kind === 'opening' ? ['openings', 'opening_id'] : target.kind === 'purchase' ? ['purchases', 'purchase_id'] : ['visits', 'winery_visit_id']; const path = `${householdId}/${parent[0]}/${target.record.id}/${createUniqueId()}-${safeName(file.name)}`; try { validatePhoto(file); const stored = await supabase.storage.from('cellar-photos').upload(path, file, { contentType: file.type, upsert: false }); if (stored.error) throw stored.error; const result = await supabase.from('photos').insert({ household_id: householdId, [parent[1]]: target.record.id, storage_path: path, original_filename: file.name, mime_type: file.type, file_size_bytes: file.size, is_hero: (target.kind === 'wine' || target.kind === 'winery') && photos.length === 0 }); if (result.error) { await supabase.storage.from('cellar-photos').remove([path]); throw result.error } await finishSuccessfulAction({ refresh: onSaved, notice: onNotice, message: 'Photo added.' }) } catch (error) { setMessage(userError(error, 'The photo could not be uploaded. Please try again.')) } finally { setBusy(false) } }
  const setHero = async (photo: PhotoRecord) => { if (!supabase || (target.kind !== 'wine' && target.kind !== 'winery')) return; setBusy(true); setMessage(''); const column = target.kind === 'wine' ? 'wine_id' : 'winery_id'; const reset = await supabase.from('photos').update({ is_hero: false }).eq('household_id', householdId).eq(column, target.record.id); const result = reset.error ? reset : await supabase.from('photos').update({ is_hero: true }).eq('household_id', householdId).eq('id', photo.id); if (result.error) setMessage(userError(result.error, 'The hero photo could not be changed.')); else await finishSuccessfulAction({ refresh: onSaved, notice: onNotice, message: 'Hero photo updated.' }); setBusy(false) }
  const remove = async (photo: PhotoRecord) => { if (!supabase || !window.confirm('Remove this photo permanently?')) return; setBusy(true); setMessage(''); const stored = await supabase.storage.from('cellar-photos').remove([photo.storagePath]); const result = stored.error ? stored : await supabase.from('photos').delete().eq('household_id', householdId).eq('id', photo.id); if (result.error) setMessage(userError(result.error, 'The photo could not be removed.')); else await finishSuccessfulAction({ refresh: onSaved, notice: onNotice, message: 'Photo deleted.' }); setBusy(false) }
  return <section className="photo-panel"><div className="photo-panel-lead"><p>Private to signed-in household members.</p>{editable && <label className="camera-button" aria-label="Add photo">＋<input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" capture="environment" disabled={busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); event.currentTarget.value = '' }} /></label>}</div>{photos.length ? <div className="gallery">{photos.map((photo) => <figure key={photo.id}><button className="gallery-image" onClick={() => onView(photo)}>{urls[photo.id] ? <img src={urls[photo.id]} alt={photo.caption ?? 'Cellar photo'} /> : <span>Loading…</span>}</button><figcaption>{photo.caption ?? (photo.isHero ? 'Hero photo' : 'Photo')}</figcaption>{editable && <div className="photo-actions">{(target.kind === 'wine' || target.kind === 'winery') && <button onClick={() => void setHero(photo)} disabled={busy || photo.isHero}>{photo.isHero ? 'Hero' : 'Make hero'}</button>}<button onClick={() => void remove(photo)} disabled={busy}>Delete</button></div>}</figure>)}</div> : <button className="photo-empty" type="button" onClick={() => inputRef.current?.click()} disabled={!editable || busy}><span>◇</span><p>{editable ? 'Add the first photo' : 'No photos yet.'}</p></button>}</section>
}

function PhotoViewer({ photo, url, onClose }: { photo: PhotoRecord; url?: string; onClose: () => void }) { return <LightboxLayer ariaLabel="Photo viewer" onDismiss={onClose}><button className="photo-viewer-close" onClick={onClose} aria-label="Close photo">×</button><figure>{url ? <img src={url} alt={photo.caption ?? 'Cellar photo'} /> : <p className="photo-viewer-loading">Loading photo…</p>}{photo.caption && <figcaption>{photo.caption}</figcaption>}</figure></LightboxLayer> }
