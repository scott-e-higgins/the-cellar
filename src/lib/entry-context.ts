...
export const tripsForDate=(data:CellarData,date:string)=>data.trips.filter(t=>Boolean(date&&t.startDate&&t.endDate)&&t.startDate<=date&&t.endDate>=date)
export function inferEntryContext(data:CellarData,wineryId:string,date:string,visitChoice:string|undefined,tripChoice:string|undefined){
 ...
 const trips=tripsForDate(data,date)
